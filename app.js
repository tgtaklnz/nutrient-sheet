...
function renderResult(food) {
  ...
  resultEl.innerHTML = `
    ...
    <p class="result-source">Source: ${
      food.fdcId
        ? `<a href="https://fdc.nal.usda.gov/food-details/${food.fdcId}/nutrients" target="_blank" rel="noopener">${food.source} (FDC #${food.fdcId})</a>`
        : `${food.source} (not yet verified against a specific FDC record)`
    }. Nutrient roles summarized from NIH Office of Dietary Supplements fact sheets.</p>
  `;
  resultEl.classList.remove('hidden');
}
